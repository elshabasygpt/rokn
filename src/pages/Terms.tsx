...
export default function Terms() {
  ...
  const pageTitle = pageData ? (isEn ? pageData.title_en : pageData.title_ar) : t('terms.title', { defaultValue: isEn ? 'Terms and Conditions' : 'الشروط والأحكام' });
  const seoDesc = pageData ? (isEn ? pageData.seo_desc_en : pageData.seo_desc_ar) : (isEn ? 'Terms and conditions for using Rokn Elryan services' : 'شروط وأحكام استخدام خدمات ركن الريان للنقل المبرد');
        ...
        <div className="bg-white rounded-3xl p-8 md:p-12 shadow-xl shadow-slate-200/50 border border-slate-100">
          {loading ? (
            ...
            <div className={`prose prose-lg max-w-none text-slate-600 ${isEn ? 'text-left' : 'text-right'}`}>
               <p>{isEn ? 'Our terms and conditions will be updated soon. Please contact support for any inquiries.' : 'سيتم تحديث الشروط والأحكام قريباً. يرجى التواصل مع الدعم الفني لأي استفسارات.'}</p>
            </div>
          )}
        </div>
      </div>
