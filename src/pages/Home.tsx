              ...
              <div className="bg-white/80 backdrop-blur-2xl p-6 md:p-8 rounded-[2rem] border border-white shadow-[0_8px_30px_rgb(0,0,0,0.06)] relative overflow-hidden">
                ...
                >
                  ...
                  <div className="mb-6 p-4 rounded-xl bg-white/60 backdrop-blur-sm border border-amber-200/50 space-y-3 text-start relative z-10 shadow-sm">
                    {breakdown.baseTruck > 0 ? (
                      ...
                      <div className="text-center py-8">
                        <div className="w-16 h-16 bg-gradient-to-br from-amber-100 to-amber-200 rounded-full flex items-center justify-center mx-auto mb-5 shadow-inner border border-amber-300">
                           <Calculator className="w-8 h-8 text-amber-600" />
                        </div>
                        <h4 className="text-xl font-black text-slate-800 mb-3">{isEn ? 'Custom Route Selection' : 'تسعيرة خاصة للمسارات المخصصة'}</h4>
                        <p className="text-sm font-bold text-slate-600 max-w-xs mx-auto leading-relaxed">{isEn ? 'This specific route requires a custom evaluation by our logistics experts to ensure the best rate.' : 'هذا المسار يحتاج لتقييم مباشر من خبراء النقل لدينا لضمان تقديم أفضل سعر لك.'}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {estimate > 0 ? (
                    ...
                    <div className="py-2 mt-4 text-center relative z-10">
                      <a href={`https://wa.me/${apiData.contact?.whatsapp || ''}?text=${encodeURIComponent(`مرحباً، أود الاستفسار عن تسعيرة النقل بـ ${truckType} تبريد (${tempType}) من ${fromCity} إلى ${toCity}.`)}`} target="_blank" rel="noopener noreferrer" className="w-full inline-flex items-center justify-center gap-3 bg-[#25D366] text-white font-black px-8 py-5 text-xl rounded-2xl hover:bg-[#128C7E] shadow-[0_10px_25px_rgba(37,211,102,0.3)] hover:shadow-[0_15px_35px_rgba(37,211,102,0.45)] hover:-translate-y-1 transition-all duration-300 group overflow-hidden relative">
                        <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                        <MessageCircle className="w-7 h-7 text-white group-hover:scale-110 transition-transform drop-shadow-md" />
                        <span>{isEn ? 'Request Quote via WhatsApp' : 'طلب تسعيرة عبر الواتساب'}</span>
                      </a>
                      <div className="flex flex-wrap items-center justify-center gap-3 mt-5">
                        <p className="text-[13px] font-black text-slate-700 flex items-center gap-1.5 bg-white/70 backdrop-blur-sm px-4 py-2 rounded-full border border-slate-200 shadow-sm">
                          <Clock className="w-4 h-4 text-amber-600" />
                          {isEn ? 'Avg Response: 5 mins' : 'الرد خلال 5 دقائق'}
                        </p>
                        <p className="text-[13px] font-black text-slate-700 flex items-center gap-1.5 bg-white/70 backdrop-blur-sm px-4 py-2 rounded-full border border-slate-200 shadow-sm">
                          <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                          {isEn ? 'Free Instant Quote' : 'تسعيرة فورية مجانية'}
                        </p>
                      </div>
                    </div>
                  )}
